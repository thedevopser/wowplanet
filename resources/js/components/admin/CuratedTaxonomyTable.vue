<template>
    <p v-if="entries.length === 0" class="text-sm text-muted">
        Aucune entrée rangée ne répond à cette recherche.
    </p>

    <div v-else class="relative overflow-x-auto" tabindex="0" role="region" aria-label="Entrées de collection déjà rangées">
        <table class="w-full text-sm">
            <caption class="sr-only">Entrées de collection déjà rangées</caption>
            <thead>
            ...
            <tbody>
                <tr v-for="entry in entries" :key="entry.id" class="border-b border-default last:border-0">
                    ...
                    <td data-role="category" class="py-2.5 pr-3" :class="entry.category ? 'text-default' : 'text-subtle'">{{ entry.category ?? '—' }}</td>
                    <td data-role="source" class="py-2.5" :class="entry.source ? 'text-default' : 'text-subtle'">{{ entry.source ?? '—' }}</td>
                </tr>
            </tbody>
        </table>
    </div>
