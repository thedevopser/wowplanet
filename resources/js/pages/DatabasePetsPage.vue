<template>
    <div class="space-y-6 py-6 sm:py-8">
        ...
        <DatabasePageHeader
            ...
            :count="displayCount"
            count-label="mascottes"
            dimension="pets"
        />

        <SearchFilter v-model:search="search" placeholder="Rechercher une mascotte…" :show-hide-toggle="false" />
        ...
        <DatabasePagination
            ...
            label="Pages des mascottes"
            @page-change="goTo"
        />

        <CatalogTable v-if="filteredItems.length" caption="Liste des mascottes" :columns="COLUMNS" :rows="visibleItems">
            <template #cell-icon="{ row }"><CollectionIcon :src="row.icon_url" :alt="row.name_fr" fallback="P" size="sm" class="text-muted" /></template>
            <template #cell-name="{ row }"><a :href="row.creature_id ? `https://www.wowhead.com/fr/npc=${row.creature_id}` : `https://www.wowhead.com/fr/search?q=${encodeURIComponent(row.name_fr)}`" target="_blank" rel="noopener" class="text-default hover:underline">{{ row.name_fr }}</a></template>
