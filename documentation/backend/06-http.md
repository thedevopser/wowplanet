...
| `arbitrateTaxonomy(Request, PendingTaxonomyEntries)` | `POST /api/admin/taxonomy/arbitrate` | Range des entrées de collection et rend l'état de l'instantané versionné. Prend `entity`, `entries`, `category` et `source` — les deux derniers nuls rangent délibérément nulle part. |
...
| `TaxonomyController` | `GET /admin/taxonomy` | `AdminTaxonomyPage` | Entrées de collection à arbitrer, et leur rangement. Accepte `?entity=` et `?search=`, et sert une tranche de 50 entrées. |
