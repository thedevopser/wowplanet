...
| Static analysis | Chemins temporaires des tests (`make temp-paths-check`) | Un `uniqid()` apparaît dans `tests/`. |
...
**Chaque test a son dossier temporaire.** Pest tourne en parallèle, et tous les processus partagent le répertoire temporaire du système. Un chemin construit avec `uniqid()` ne dépend que de l'horloge : deux processus qui démarrent un test à la même microseconde obtiennent le même dossier, et le premier qui termine l'efface sous l'autre. C'est ce qui a fait tomber une fois `ImportPipelineTest`, dont l'instantané de taxonomie disparaissait avant l'étape Mascottes. Un chemin temporaire de test passe donc par `testTempPath()`, déclaré dans `tests/Pest.php`, qui le range sous `wowplanet-test-<TEST_TOKEN>/` et le termine par un aléa. `make temp-paths-check` échoue sur tout `uniqid()` sous `tests/`.
